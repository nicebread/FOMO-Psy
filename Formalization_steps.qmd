...
2. Choose your [starting point](sessions/S8-Candidate_theories#prototypical-formalization-styles):
...
5. Create your [*Construct Table*](lectures/VAST/VAST_Application.qmd#construct-table)
...
12. For the final report:
    1.  Copy the tables as PDF files from the Google doc into the `/doc` subfolder of your repository, so that it is really self-contained (i.e., all relevant files are bundled in the project folder).
    2.  Export a PNG of the final VAST display to the `/doc` subfolder of your repository
    3.  (If not done yet: Assign a version number)
    4.  Create a release on Github; the tag name should contain the final version number (e.g., `v0.8.3`).
    5.  In your manuscript, explicitly report the version number your report is referring to and provide a link to the release.
