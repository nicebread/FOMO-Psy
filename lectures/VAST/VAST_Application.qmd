...
::: {.smallest}
- A: **Narrative Theory Reconstruction (NTR)**
  ...
  - *Of only minor importance*: 
    - Whether the phenomenon is actually robust.
  ...
  - This approach focuses on *showing the gaps* (without fixing them)
- B: **Theory Construction Methodology (TCM)**
...
- This approach is more about *filling the gaps* (instead of just showing them).
...
1. **Generalizability** (cf. [UTOS framework](../Intro1/FOMO-Psy_1_Theory_crisis#generalization)): The effect has been shown for different [U]{.hl}nits, with different operationalizations of [T]{.hl}reatments and [O]{.hl}utcomes, and in multiple [S]{.hl}ettings.
...
## Interlude: Elements of a theory

The *elements* of a theory are:

- *concepts* (i.e., categories or dimensions that are applicable to certain objects)
- *relationships* (typically causal) among these concepts

::: footer
Freyer, N., Lange, J., Leising, D., Musfeld, P., & Schönbrodt, F. D. (2025, July 22). Indicators for the Assessment of Theoretical Rigor. [https://doi.org/10.31234/osf.io/e7tdc_v2](https://doi.org/10.31234/osf.io/e7tdc_v2)
:::
...
- Collect quotes of definitions of constructs and their relationships from the literature. 
  - If there is no quote for a construct that is still needed: Create a provisional definition based on your understanding, without any quote, and mark it as such.
...
<table>
  <thead>
    <tr>
      ...
      <th>Element type</th>
      <th>Rel. type</th>
      <th>Short name / associated concept</th>
      <th>ID</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td rowspan="4">Fischer et al. (2011), p. 517</td>
      <td rowspan="4">“The bystander effect refers to the phenomenon that an individual's likelihood of helping decreases when passive bystanders are present in a critical situation.”</td>
      <td>likelihood of helping</td>
      <td>C</td>
      <td>n</td>
      <td>p(help)</td>
      <td>A1</td>
    ...
    <tr>
      <td>number of passive bystanders</td>
      <td>C</td>
      <td>n</td>
      <td>NOPB</td>
      <td>A2</td>
    ...
    <tr>
      <td>p(help) <i>decreases</i> when NOPB &gt; 0, given critSit</td>
      <td>HOC</td>
      <td>p</td>
...
- *Element Type* is one of: [C]{.hl}oncept, [R]{.hl}elationship, [HOC]{.hl}: Higher-order Concept.
- *Relationship Type*: One of the VAST relationship types (n, p, i, r, c, …)
...
- Use the ID from the *Construct Table* as subscript for each relationship. This way your claims in the VAST display can be easily backlinked to the original sources in the literature.
  - The small "A" subscript indicates that all relationships in this display are derived from the quote with ID "A" in the *Construct Table*.
:::
